    ...
    _.each(list, (dest, src) => {
      _.castArray(dest).forEach((_dest) => {
        ...
        if (alreadyExist && fromEntity) {
          ...
          console.log(
            `Copied "${_dest}"` + (fromEntity ? ` from ${fromEntity}` : "")
          );
        }
      });
    });
...
const generateRuntimeConfig = (
  ...
  if (tailwindVersion === 3) {
    ...
    runtimeConfig.tailwind = JSON.parse(
      fsSync.readFileSync(tailwind3ConfigFile, "utf-8")
    );
  } else if (tailwindVersion === 4) {
    const tailwindScanPackagePrefix = [
      "@xgent/react",
      "@xgent/grafton",
      "@xgent/ui-",
    ];
    ...
    subModules.forEach((subModule) => {
      if (subModule.mode === "package") {
        scanSources.push(
          "../node_modules/" + subModule.importPath + "/dist/*.es*.js"
        );
        scanSources.push(
          "../node_modules/" + subModule.importPath + "/dist/**/*.css"
        );
      }
    });
    ...
    fsSync.writeFileSync(
      "./src/runtime.css",
      scanSources.map((line) => `@source "${line}";`).join("\n"),
      "utf-8"
    );
  ...
  async function buildRoutes(rootDir, currentDir, isRoot, parentPath = "/") {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const isIndex =
          entry.name.endsWith("index.jsx") ||
          entry.name.endsWith("index.lazy_.jsx");
...
`;
  ...
  async function copyPages(routesPath, runtimePagesPath, key) {
    ...
    const cwd = process.cwd();
    console.log(
      `Sub-module pages copied ${path.relative(
        cwd,
        routesPath
      )} -> ${path.relative(cwd, runtimePagesPath)}`
    );
  }
  ...
  return {
    ...
    async configureServer(server) {
      ...
      // 防止极少数情况下 httpServer 已经在 listening
    ...
    .map((route) => {
      ...
      if (route.children && route.children.length > 0) {
        ...
        if (
          ...
          Object.keys(orderedRoute).length === 1 &&
          "path" in orderedRoute
        ) {
